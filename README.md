<div align="center">

<h1>
  <span style="letter-spacing: 8px;">P O S E I C</span>
</h1>

<p>
  <strong>developer / builder</strong><br/>
  <code>React</code> &nbsp;•&nbsp; <code>TypeScript</code> &nbsp;•&nbsp; <code>Discord.js</code> &nbsp;•&nbsp; <code>Linux</code>
</p>

<p>
  <em>ne duyduysan daha fazlası.</em>
</p>

<br/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=16&pause=1400&color=818CF8&center=true&vCenter=true&repeat=true&width=520&height=32&lines=React+%26+Next.js;TypeScript+%2F+Prisma;Discord.js+v14;Linux+%26+Altyap%C4%B1;Building+things+that+actually+ship." alt="Poseic Stack" />
</a>

<br/><br/>

<p align="center">
  <a href="https://www.poseic.xyz">
    <img src="https://img.shields.io/badge/WEBSITE-poseic.xyz-090a0f?style=for-the-badge&logo=googlechrome&logoColor=38bdf8&labelColor=050608&color=1e293b" alt="Website" />
  </a>
  &nbsp;
  <a href="https://discord.gg/sql">
    <img src="https://img.shields.io/badge/COMMUNITY-ProjectSQL-090a0f?style=for-the-badge&logo=discord&logoColor=818cf8&labelColor=050608&color=1e293b" alt="ProjectSQL" />
  </a>
  &nbsp;
  <a href="https://github.com/poseicc">
    <img src="https://img.shields.io/badge/GITHUB-poseicc-090a0f?style=for-the-badge&logo=github&logoColor=ffffff&labelColor=050608&color=1e293b" alt="GitHub" />
  </a>
</p>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:06070a,25:1e1b4b,50:6366f1,75:1e1b4b,100:06070a&height=2&section=header" width="100%" alt="" />

</div>

### `Pişiriyorum`

<table>
  <tr>
    <td width="50%" valign="top">
      <h4>🌐 Web & Arayüz</h4>
      <ul>
        <li><b>React & Next.js:</b> Turbopack, ISR ve bileşen mimarisi.</li>
        <li><b>Temiz Tasarım:</b> Hızlı yüklenen, sade ve tutarlı arayüzler.</li>
        <li><b>Performans:</b> Hafif kod tabanı ve optimize asset akışı.</li>
      </ul>
      <p><code>React</code> • <code>Next.js</code> • <code>Tailwind</code></p>
    </td>
    <td width="50%" valign="top">
      <h4>🛡️ Veri & Tip Güvenliği</h4>
      <ul>
        <li><b>TypeScript & Prisma:</b> Tip güvenli veri modelleri ve güvenli sorgu akışları.</li>
        <li><b>API Tasarımı:</b> Katı doğrulama şemaları ve temiz hata yönetimi.</li>
        <li><b>Süreklilik:</b> Okunabilir ve sürdürülebilir mimari.</li>
      </ul>
      <p><code>TypeScript</code> • <code>Prisma ORM</code> • <code>SQLite / PostgreSQL</code></p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4>🤖 Discord Sistemleri</h4>
      <ul>
        <li><b>Discord.js v14:</b> Slash komutları, dinamik modallar ve butonlar.</li>
        <li><b>Moderasyon:</b> Olay tabanlı loglama ve filtre mekanizmaları.</li>
        <li><b>Durum Senkronizasyonu:</b> Rol ve yetki otomasyonları.</li>
      </ul>
      <p><code>Node.js</code> • <code>Discord.js v14</code> • <code>Event-driven</code></p>
    </td>
    <td width="50%" valign="top">
      <h4>🖥️ Altyapı & Sunucu</h4>
      <ul>
        <li><b>Linux VDS:</b> Debian/Ubuntu ortamı ve SSH anahtar güvenliği.</li>
        <li><b>Servis Yönetimi:</b> systemd ile süreç kontrolü ve log izleme.</li>
        <li><b>Minecraft Altyapısı:</b> PaperMC üzerinde performans ve sunucu yapılandırması.</li>
      </ul>
      <p><code>Linux</code> • <code>Bash</code> • <code>systemd</code> • <code>PaperMC</code></p>
    </td>
  </tr>
</table>

<br/>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=0:06070a,25:1e1b4b,50:6366f1,75:1e1b4b,100:06070a&height=2&section=header" width="100%" alt="" />
</div>

### `Biliyorum`

<div align="center">

<table border="0">
  <tr>
    <td align="right"><b>Frontend</b></td>
    <td><img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,html,css" alt="Frontend" /></td>
  </tr>
  <tr>
    <td align="right"><b>Backend & Veri</b></td>
    <td><img src="https://skillicons.dev/icons?i=nodejs,discordjs,prisma,py,sqlite" alt="Backend & Veri" /></td>
  </tr>
  <tr>
    <td align="right"><b>Sistem & DevOps</b></td>
    <td><img src="https://skillicons.dev/icons?i=linux,ubuntu,nginx,git,github,bash" alt="Sistemler" /></td>
  </tr>
  <tr>
    <td align="right"><b>Araçlar</b></td>
    <td><img src="https://skillicons.dev/icons?i=vscode,docker" alt="Araçlar" /></td>
  </tr>
</table>

</div>

<br/>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=0:06070a,25:1e1b4b,50:6366f1,75:1e1b4b,100:06070a&height=2&section=header" width="100%" alt="" />
</div>

### `Tutturuyorum`

<div align="center">

<p align="center">
  <img src="https://github-readme-3d-contrib.vercel.app/profile-gitblock.svg?username=poseicc" alt="3D Voksel Katkı Takvimi" width="95%" />
</p>

<br/>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=poseicc&show_icons=true&theme=tokyonight&hide_border=true&bg_color=06070a&title_color=818cf8&icon_color=38bdf8&text_color=94a3b8&border_radius=10&include_all_commits=true&count_private=true" height="150" alt="GitHub Stats" />
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=poseicc&layout=compact&theme=tokyonight&hide_border=true&bg_color=06070a&title_color=818cf8&text_color=94a3b8&border_radius=10&langs_count=6" height="150" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=poseicc&theme=tokyonight-duo&hide_border=true&background=06070a&ring=818cf8&fire=38bdf8&currStreakLabel=818cf8&sideLabels=94a3b8&dates=64748b&border_radius=10" alt="GitHub Streak" />
</p>

</div>

<br/>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=0:06070a,25:1e1b4b,50:6366f1,75:1e1b4b,100:06070a&height=2&section=header" width="100%" alt="" />
</div>

<div align="center">

### `Bağlanıyoruz`

<br/>

<p align="center">
  <a href="https://www.poseic.xyz">
    <img src="https://img.shields.io/badge/WEBSITE-poseic.xyz-06070a?style=for-the-badge&logo=google-chrome&logoColor=38bdf8&labelColor=0b0e14&color=1e293b" alt="Website" />
  </a>
  &nbsp;&nbsp;
  <a href="https://discord.gg/sql">
    <img src="https://img.shields.io/badge/COMMUNITY-ProjectSQL-06070a?style=for-the-badge&logo=discord&logoColor=818cf8&labelColor=0b0e14&color=1e293b" alt="Discord" />
  </a>
  &nbsp;&nbsp;
  <a href="https://www.youtube.com/@Poseic">
    <img src="https://img.shields.io/badge/YOUTUBE-@Poseic-06070a?style=for-the-badge&logo=youtube&logoColor=ff0000&labelColor=0b0e14&color=1e293b" alt="YouTube" />
  </a>
<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:06070a,50:0e1320,100:4f46e5&height=80&section=footer" width="100%" alt="" />

<p align="center">
  <sub>Poseic • 2026</sub>
</p>

</div>
